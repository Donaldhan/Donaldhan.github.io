...
* [ Curator节点监听](#curator节点监听)
    ...
    * [子一级目录监听器PathChildrenCache](#子一级目录监听器pathchildrencache)
* [总结](#总结)
...
```
从上面可以看出，子目录监听器PathChildrenCache，主要成员变量为客户端框架实现CuratorFramework，子路径监听器容器ListenerContainer<PathChildrenCacheListener>，
及事件执行器CloseableExecutorService，事件操作集Set<Operation>。
...
```
从上面可以看出连接状态监听器ConnectionStateListener，根据连接状态来添加事件EventOperation和刷新RefreshOperation操作到操作集。
先来看事件操作
...
```
从上面可以看出，重新构建操作，主要是刷新缓存路径数据，并注册刷新操作。
小节一下：
